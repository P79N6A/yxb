...
@Service
public class DeleteLikeServiceImpl extends ServiceImpl<LikeMapper, Like> implements DeleteLikeService {
    @Autowired
    ...
    @Override
    public Result<Map<String, Object>> deleteLike(String beLikedId, String token) {
        String memberId = redisTemplateUtils.getStringValue(token);
        if (StringUtil.isEmpty(memberId)){
            return new Result<>(Code.FAIL, "token为空!", null, Code.IS_ALERT_YES);
        }
        EntityWrapper<Like> likeEntityWrapper = new EntityWrapper<>();
        likeEntityWrapper.eq("be_liked_id", beLikedId);
        likeEntityWrapper.eq("member_id", memberId);

        Integer delete = likeMapper.delete(likeEntityWrapper);
